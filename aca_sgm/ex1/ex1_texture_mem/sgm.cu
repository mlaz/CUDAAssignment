...
/* textures */
texture<int, cudaTextureType2D, cudaReadModeElementType> texr;
texture<int, cudaTextureType2D, cudaReadModeElementType> texl;
texture<int, cudaTextureType2D, cudaReadModeElementType> texc;
                                  ...
                                  int nx, int ny, int disp_range )
{
  ...
  int x = ((blockIdx.x / disp_range) * 32) + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  int d = blockIdx.x % disp_range;
  if ( (y < ny) && (d < disp_range) && (x < nx))
    {
      COSTS(x,y,d) = 255u;
      if (x >= d)
        //COSTS(x,y,d) = abs( LEFT_IMAGE(x,y) - RIGHT_IMAGE(x-d,y) );
        COSTS(x,y,d) = abs( tex2D(texl, x, y) - tex2D(texr, x-d, y) );
    }
}
                 ...
                 const int w, const int h, const int disp_range )
{
  ...
  cudaArray* d_tx_left_image;
  cudaArray* d_tx_right_image;
  cudaMallocArray(&d_tx_left_image, &channelDesc, nx, ny);
  cudaMallocArray(&d_tx_right_image, &channelDesc, nx, ny);
  cudaMemcpyToArray(d_tx_left_image, 0, 0, h_leftIm, image_size , cudaMemcpyHostToDevice);
  cudaMemcpyToArray(d_tx_right_image, 0, 0, h_rightIm, image_size , cudaMemcpyHostToDevice);
  cudaBindTextureToArray(texl, d_tx_left_image);
  cudaBindTextureToArray(texr, d_tx_right_image);
  ...
  int block_x = 32;
  int block_y = 16;
  int grid_x = ceil((float) nx / block_x) * disp_range;
